...
- `src/webview/scrollMemory.ts` — the pure half of scroll persistence: `readEntries` (defensive, because persisted state outlives the code that wrote it), `rememberScroll`, `recallScroll`, and the `MAX_ENTRIES` cap. Entries live in the webview's own `setState()`/`getState()`, the only thing VS Code carries across a sidebar teardown, and are keyed by page id — which is why `init` carries a `pageId`: the same message means both "the view came back" and "we moved to another note", and only the first should restore. Editor position is a **document offset**, not a pixel offset, so a sidebar resized while hidden still lands on the same line; the reader has no offsets and keeps pixels. Merging, not replacing, is what lets the editor and the reader each report only their own half. `index.ts` debounces saves at 200 ms and flushes them on `blur`, `visibilitychange`, `pagehide`, mode switches, and page switches — the debounce is a throttle, not a grace period, and a collapsing sidebar does not wait.
...
- `harness.html` — standalone HTML with mocked `acquireVsCodeApi` that loads `dist/webview.js`. Used by Playwright tests to exercise the webview outside VS Code. Its `getState`/`setState` are backed by `sessionStorage`, which makes `page.reload()` a faithful stand-in for the sidebar teardown that destroys a webview and hands its persisted state to the replacement.
